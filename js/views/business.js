...
function customerOrders() {
  ...
  customerOverview = function () {
    ...
    const ownApplications = fxAllBindingReviewRequests(true).filter(item => item.customer === fxCurrentCustomer().name);
  ...
  customerWithdrawals = function () {
    ...
    return `<div class="page">${pageHeader("撤回申请", "选择产品的一个或多个已绑码段发起撤回申请", actions)}<div class="table-shell"><div class="table-scroll"><table><thead><tr><th>申请编号</th><th>产品名称</th><th>产品批次</th><th>撤回码段</th><th>撤回原因</th><th>${fxCustomerDateHeader("申请时间", "withdrawal")}</th><th>${fxTableSelectHeader("申请状态", "customerWithdrawalStatus", ["全部状态", "待审批", "已通过", "已驳回"], state.customerWithdrawalStatus)}</th><th>处理说明</th></tr></thead><tbody>${rows.length ? rows.map(item => { const product = products.find(row => row.name === item.product && row.company === item.customer && (!item.batch || row.batch === item.batch)); return `<tr><td class="mono">${fxEscape(item.no)}</td><td>${fxEscape(item.product)}</td><td class="mono">${fxEscape(item.batch || product?.batch || "—")}</td><td>${fxWithdrawalRangeMarkup(item)}</td><td>${fxEscape(item.reason)}</td><td>${fxEscape(item.time)}</td><td>${status(item.status)}</td><td>${fxEscape(item.rejectReason || (item.status === "已通过" ? "申请码段已重置" : "等待运营方审批"))}</td></tr>`; }).join("") : fxEmpty(8, "暂无撤回申请")}</tbody></table></div>${pagination(rows.length)}</div></div>`;
  };
  ...
  function fxCustomerDetailPage() {
    ...
    const orderRows = ownOrders.map(order => {
      ...
      return `<tr><td><button class="text-action mono" data-action="fx-focus-order" data-no="${fxEscape(order.no)}">${fxEscape(order.no)}</button></td><td>${fxEscape(order.createdAt || order.created || "—")}</td><td>${formatNumber(order.total)}</td><td>${formatNumber(order.active)}</td><td>${pendingCell}</td><td>${formatNumber(remaining)}</td><td class="action-column"><div class="table-actions">${remaining > 0 ? `<button class="text-action" data-action="fx-ops-bind-order" data-no="${fxEscape(order.no)}">绑定产品</button>` : ""}<button class="text-action" data-action="fx-order-detail" data-no="${fxEscape(order.no)}">详情</button></div></td></tr>`;
  ...
  function fxOrderDetailPage() {
    ...
    const applications = fxNewestRows(fxAllBindingReviewRequests(true).filter(request => request.orderNo === order.no && request.customer === order.customer), request => request.time);
    ...
    const applicationRecords = applications.map(request => {
      ...
      if (!activation && combined && request.status === "已通过") {
        const candidates = activations.filter(item => !matchedActivations.has(item) && item.product === request.product && (!request.batch || item.batch === request.batch));
        activation = candidates.find(item => request.range && item.range === request.range)
          || candidates.find(item => Number(item.amount || 0) === Number(request.amount || 0));
      }
      ...
      const recordProduct = products.find(item => Number(item.id) === Number(request.productId) || (item.name === (activation?.product || request.product) && (!request.batch || item.batch === (activation?.batch || request.batch))));
    ...
    const activationRecords = activations.filter(item => !matchedActivations.has(item)).map(item => {
      const product = products.find(productItem => productItem.name === item.product && productItem.company === order.customer && (!item.batch || productItem.batch === item.batch))
        || products.find(productItem => productItem.name === item.product && productItem.company === order.customer);
  ...
  modalMarkup = function () {
    ...
    if (state.modal === "fx-customer-bind-order") {
      ...
      return fxModalShell("绑定已激活产品", order.no, `<div class="form-grid">${fxBindProductPicker("fx-customer-bind-product", activated, true)}<div class="field full"><label class="required">本次绑定数量</label><input id="fx-customer-bind-amount" type="number" min="1" max="${remaining}" value="${Math.min(1000, remaining)}"><span class="field-help">剩余可用码段：${formatNumber(remaining)} 枚。绑定后对应连续码段立即激活，并写入产品绑定记录。</span></div></div>`, `<button class="button" data-action="close-modal">取消</button><button class="button primary" data-action="fx-confirm-customer-bind">确认绑定</button>`);
    }
