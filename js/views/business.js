...
function customerOrders() {
  ...
  function fxOrderDetailPage() {
    ...
    return `<div class="page order-detail-page">${pageHeader(detailLabel, "", actions)}<section class="detail-section"><h3>订单信息</h3><dl class="detail-grid order-detail-grid"><div class="detail-item order-detail-half"><dt>订单号</dt><dd class="mono">${fxEscape(order.no)}</dd></div><div class="detail-item order-detail-half"><dt>客户名称</dt><dd>${fxEscape(order.customer || "—")}</dd></div><div class="detail-item order-detail-half"><dt>分配时间</dt><dd>${fxEscape(order.allocatedAt || order.createdAt || order.created || "—")}</dd></div><div class="detail-item order-detail-half"><dt>操作人</dt><dd>${fxEscape(order.allocatedBy || "—")}</dd></div><div class="detail-item order-detail-wide"><dt>序列号范围</dt><dd class="mono">${fxEscape(order.range)}</dd></div><div class="detail-item"><dt>订单码量</dt><dd>${formatNumber(order.total)}</dd></div><div class="detail-item"><dt>已激活</dt><dd>${formatNumber(order.active)}</dd></div><div class="detail-item"><dt>绑定申请中</dt><dd>${formatNumber(pendingAmount)}</dd></div><div class="detail-item"><dt>剩余可用</dt><dd>${formatNumber(availableAmount)}</dd></div><div class="detail-item order-detail-wide"><dt>订单备注</dt><dd class="order-note-detail">${fxEscape(order.note || "—")}</dd></div></dl></section><section class="detail-section"><div class="detail-title-row"><h3>产品绑定记录</h3></div>${bindingToolbar}<div class="table-shell"><div class="table-scroll"><table class="order-binding-table" data-sequence="off"><thead>${bindingHeader}</thead><tbody>${bindingRows}</tbody></table></div></div></section></div>`;
  }
