...
function customerOrders() {
  ...
  function fxCustomerDetailPage() {
    ...
    const orderRows = ownOrders.map(order => {
      ...
      return `<tr><td><button class="text-action mono" data-action="fx-order-detail" data-no="${fxEscape(order.no)}">${fxEscape(order.no)}</button></td><td>${fxEscape(order.allocatedAt || order.createdAt || order.created || "—")}</td><td>${formatNumber(order.total)}</td><td>${formatNumber(order.active)}</td><td>${pendingCell}</td><td>${formatNumber(remaining)}</td><td class="action-column"><button class="text-action" data-action="fx-order-detail" data-no="${fxEscape(order.no)}">详情</button></td></tr>`;
    ...
    return `<div class="page customer-detail-page">${pageHeader("客户详情", "", actions)}<section class="detail-section"><h3>客户完整资料</h3><dl class="detail-grid"><div class="detail-item"><dt>客户名称</dt><dd>${fxEscape(item.name)}</dd></div><div class="detail-item"><dt>登录账号</dt><dd class="mono">${fxEscape(item.account)}</dd></div><div class="detail-item"><dt>联系电话</dt><dd>${fxEscape(item.phone)}</dd></div><div class="detail-item"><dt>账号状态</dt><dd>${status(item.status)}</dd></div><div class="detail-item"><dt>营业执照</dt><dd>${fxEscape(fxFileName(item.license) || "未上传")}</dd></div><div class="detail-item"><dt>法人身份证</dt><dd>${fxEscape(fxFileName(item.legalId) || "未上传")}</dd></div><div class="detail-item"><dt>已激活产品</dt><dd>${formatNumber(activeProducts.length)}</dd></div></dl></section><section class="detail-section"><div class="detail-title-row"><h3>已分配码段记录</h3></div><div class="table-scroll"><table><thead><tr><th>分配记录号</th><th>分配时间</th><th>分配数量</th><th>已激活</th><th>绑定申请中</th><th>剩余可用</th><th class="action-column">操作</th></tr></thead><tbody>${orderRows}</tbody></table></div></section></div>`;
  }
  ...
  function fxInventoryDetailPage() {
    ...
    const allocationRows = allocations.length ? allocations.map(item => `<tr><td>${fxEscape(item.customer || item.customerName || "—")}</td><td class="mono">${fxEscape(item.no || "—")}</td><td class="mono">${fxEscape(item.range || "—")}</td><td>${formatNumber(item.total || item.amount || 0)}</td><td>${fxEscape(item.allocatedAt || item.createdAt || item.created || "—")}</td><td>${fxEscape(item.allocatedBy || "—")}</td><td><button class="text-action" data-action="fx-order-detail" data-no="${fxEscape(item.no || "")}">详情</button></td></tr>`).join("") : fxEmpty(7, "未找到分配记录");
    ...
    return `<div class="page inventory-detail-page">${pageHeader("库存码段详情", "", actions)}<section class="detail-section inventory-batch-summary"><h3>码段信息</h3><dl class="detail-grid"><div class="detail-item"><dt>序列号范围</dt><dd class="mono">${fxEscape(batch.range || "—")}</dd></div><div class="detail-item"><dt>生成时间</dt><dd>${fxEscape(batch.createdAt || batch.created || "—")}</dd></div><div class="detail-item"><dt>生成数量</dt><dd>${formatNumber(batch.total)} 枚</dd></div><div class="detail-item"><dt>已分配</dt><dd>${formatNumber(fxCodeBatchAllocatedAmount(batch))} 枚</dd></div><div class="detail-item"><dt>剩余库存</dt><dd>${formatNumber(fxCodeBatchRemainingAmount(batch))} 枚</dd></div><div class="detail-item"><dt>分配状态</dt><dd>${status(fxCodeBatchStatus(batch))}</dd></div><div class="detail-item"><dt>输出尺寸</dt><dd>${fxEscape(batch.size || "—")}</dd></div><div class="detail-item"><dt>备注</dt><dd>${fxEscape(batch.note || "—")}</dd></div></dl></section><section class="detail-section inventory-allocation-records"><div class="detail-title-row"><h3>客户分配记录</h3></div><div class="toolbar inventory-allocation-toolbar"><div class="filters compact-search-filters">${fxFilterInput("fx-inventory-allocation-customer-search", state.inventoryAllocationCustomerFilter, "搜索客户名称")}${fxFilterInput("fx-inventory-allocation-order-search", state.inventoryAllocationOrderFilter, "搜索订单号")}<button type="button" class="button small list-reset-button" data-action="fx-reset-list" data-reset-context="inventory-allocations">${icon("rotate-ccw", "↻")}重置</button></div></div><div class="table-shell"><div class="table-scroll"><table class="inventory-allocation-table" data-sequence="off"><thead><tr><th>客户名称</th><th>订单号</th><th>分配码段</th><th>${fxSortHeader("数量", "inventoryAllocation", "total")}</th><th>${fxCustomerDateHeader("分配时间", "inventoryAllocation")}</th><th>操作人</th><th class="action-column">操作</th></tr></thead><tbody>${allocationRows}</tbody></table></div></div></section></div>`;
  }
  ...
  function fxReviewDetailPage() {
    ...
    return `<div class="page review-detail-page">${pageHeader("绑定审核详情", "同时核对产品资料、分配记录、申请数量与绑定码段", actions)}<div class="review-detail-content">${body}</div></div>`;
  }
  ...
  function fxOrderDetailPage() {
    ...
    return `<div class="page order-detail-page">${pageHeader(detailLabel, "", actions)}<section class="detail-section"><h3>订单信息</h3><dl class="detail-grid"><div class="detail-item"><dt>订单号</dt><dd class="mono">${fxEscape(order.no)}</dd></div><div class="detail-item"><dt>客户名称</dt><dd>${fxEscape(order.customer || "—")}</dd></div><div class="detail-item"><dt>来源库存码段</dt><dd class="mono">${fxEscape(fxInventoryBatches().find(item => item.no === order.sourceBatchNo)?.range || "历史码段")}</dd></div><div class="detail-item"><dt>分配时间</dt><dd>${fxEscape(order.allocatedAt || order.createdAt || order.created || "—")}</dd></div><div class="detail-item"><dt>操作人</dt><dd>${fxEscape(order.allocatedBy || "—")}</dd></div><div class="detail-item"><dt>序列号范围</dt><dd class="mono">${fxEscape(order.range)}</dd></div><div class="detail-item"><dt>订单码量</dt><dd>${formatNumber(order.total)}</dd></div><div class="detail-item"><dt>已激活</dt><dd>${formatNumber(order.active)}</dd></div><div class="detail-item"><dt>绑定申请中</dt><dd>${formatNumber(pendingAmount)}</dd></div><div class="detail-item"><dt>剩余可用</dt><dd>${formatNumber(availableAmount)}</dd></div><div class="detail-item"><dt>订单备注</dt><dd class="order-note-detail">${fxEscape(order.note || "—")}</dd></div></dl></section><section class="detail-section"><div class="detail-title-row"><h3>产品绑定记录</h3></div>${bindingToolbar}<div class="table-shell"><div class="table-scroll"><table class="order-binding-table" data-sequence="off"><thead>${bindingHeader}</thead><tbody>${bindingRows}</tbody></table></div></div></section></div>`;
  }
  ...
  modalMarkup = function () {
    ...
    if (state.modal === "fx-code-allocation") {
      ...
      return fxModalShell("分配码段给客户", "由运营端执行", body, `<button class="button" data-action="close-modal">取消</button><button class="button primary" data-action="fx-confirm-code-allocation">确认分配</button>`);
    }
    if (state.modal === "fx-order-bind-requests") {
      ...
      const rows = requests.length
        ...
        : `<tr><td colspan="7">当前分配记录暂无待审批绑定申请</td></tr>`;
    ...
    if (state.modal === "fx-customer-bind-choice") {
      ...
      const remaining = order ? fxOrderAvailableAmount(order) : 0;
      if (!order || remaining < 1) return fxModalShell("申请绑定产品", "当前分配记录不存在或已无可用码量。", `<div class="empty"><p>没有可申请的可用码量</p></div>`, `<button class="button primary" data-action="close-modal">关闭</button>`);
    ...
    if (state.modal === "fx-withdraw-decision") { const item = withdrawals[state.selectedWithdrawalIndex]; const reject = state.modalData?.decision === "reject"; const segments = fxResolvedWithdrawalSegments(item); const amount = item.requestedAmount || segments.reduce((sum, segment) => sum + Number(segment.amount || 0), 0); return fxModalShell(reject ? "驳回撤回申请" : "确认通过撤回申请", reject ? "填写驳回原因并通知客户。" : "请核对以下码段；确认后将逐段重置，且无法在本次审批中撤销。", `<div class="confirm-list"><div class="confirm-row"><span>产品</span><strong>${fxEscape(item.product)}</strong></div><div class="confirm-row"><span>客户</span><strong>${fxEscape(item.customer)}</strong></div><div class="confirm-row"><span>撤回范围</span><strong>${item.scope === "segments" ? "选定已绑码段" : "整产品关联码段"}</strong></div><div class="confirm-row"><span>撤回码段</span>${fxWithdrawalRangeMarkup(item)}</div><div class="confirm-row"><span>撤回数量</span><strong>${formatNumber(amount)} 枚</strong></div><div class="confirm-row"><span>申请原因</span><strong>${fxEscape(item.reason)}</strong></div></div>${reject ? `<div class="field modal-field"><label class="required">驳回原因</label><textarea id="fx-withdraw-reject-reason"></textarea></div>` : ""}`, `<button class="button" data-action="close-modal">取消</button><button class="button ${reject ? "danger" : "primary"}" data-action="fx-confirm-withdraw-decision">确认${reject ? "驳回" : "通过并重置"}</button>`); }
