...
function customerOrders() {
  ...
  function fxStandardFieldMediaRows(module, field) {
    ...
    const upload = state.editorReadonly ? "" : `<label class="button custom-field-upload">${icon(acceptsPdf ? "paperclip" : "image-plus", "+")}选择${noun}<input type="file" data-fx-standard-upload="${field.mediaKey}" data-fx-standard-types="${acceptsPdf ? "image,pdf" : "image"}" accept="${accept}" multiple></label>`;
    const append = state.editorReadonly ? "" : `<label class="editor-inline-media-add">${icon("plus", "+")}<span>添加</span><input type="file" data-fx-standard-upload="${field.mediaKey}" data-fx-standard-types="${acceptsPdf ? "image,pdf" : "image"}" accept="${accept}" multiple></label>`;
    if (state.editorReadonly && !files.length) return "";
    return `<div class="custom-field-files standard-field-media" aria-label="${fxEscape(field.mediaLabel || `${field.label}${noun}`)}">${files.length ? `<div class="scan-media-list editor-inline-media-list">${previews}${append}</div>` : `${upload}<div class="custom-field-file-empty">尚未添加${noun}</div>`}</div>`;
  }
  ...
  function fxCustomFileRows(module, row, rowIndex, nameReady = true) {
    ...
    const disabled = nameReady ? "" : "disabled"; const disabledClass = nameReady ? "" : " is-disabled";
    const upload = state.editorReadonly ? "" : `<label class="button custom-field-upload${disabledClass}" aria-disabled="${nameReady ? "false" : "true"}">${icon("paperclip", "□")}选择图片/附件<input type="file" data-fx-custom-upload="${module}:${rowIndex}" accept="image/*,.pdf" multiple ${disabled}></label>`;
    const append = state.editorReadonly ? "" : `<label class="editor-inline-media-add${disabledClass}" aria-disabled="${nameReady ? "false" : "true"}">${icon("plus", "+")}<span>添加</span><input type="file" data-fx-custom-upload="${module}:${rowIndex}" accept="image/*,.pdf" multiple ${disabled}></label>`;
    if (state.editorReadonly && !files.length) return "";
    return `<div class="custom-field-files">${files.length ? `<div class="scan-media-list editor-inline-media-list">${previews}${append}</div>` : `${upload}<div class="custom-field-file-empty">${nameReady ? "尚未添加图片或附件" : "填写字段名称后可添加图片或附件"}</div>`}</div>`;
  }
  ...
  customerWithdrawals = function () {
    ...
    return `<div class="page">${pageHeader("撤回申请", "仅支持对已激活产品发起全量撤回", `<button class="button primary" data-action="fx-open-withdraw">${icon("plus", "+")}发起申请</button>`)}<div class="table-shell"><div class="table-scroll"><table><thead><tr><th>申请编号</th><th>产品名称</th><th>产品批次</th><th>撤回原因</th><th>${fxCustomerDateHeader("申请时间", "withdrawal")}</th><th>${fxTableSelectHeader("申请状态", "customerWithdrawalStatus", ["全部状态", "待审批", "已通过", "已驳回"], state.customerWithdrawalStatus)}</th><th>处理说明</th></tr></thead><tbody>${rows.length ? rows.map(item => { const product = products.find(row => row.name === item.product && row.company === item.customer && (!item.batch || row.batch === item.batch)); return `<tr><td class="mono">${fxEscape(item.no)}</td><td>${fxEscape(item.product)}</td><td class="mono">${fxEscape(item.batch || product?.batch || "—")}</td><td>${fxEscape(item.reason)}</td><td>${fxEscape(item.time)}</td><td>${status(item.status)}</td><td>${fxEscape(item.rejectReason || (item.status === "已通过" ? "关联码已重置" : "等待运营方审批"))}</td></tr>`; }).join("") : fxEmpty(7, "暂无撤回申请")}</tbody></table></div>${pagination(rows.length)}</div></div>`;
  };
  ...
  modalMarkup = function () {
    ...
    if (state.modal === "fx-order-bind-requests") {
      ...
      const rows = requests.length
        ? requests.map(request => `<tr><td>${request.source === "product-review" ? "新建产品并绑定" : "已有产品绑定"}</td><td>${fxEscape(request.product)}</td><td class="mono">${fxEscape(request.batch || "—")}</td><td class="mono">${fxEscape(request.range || "审核通过后分配")}</td><td>${formatNumber(request.amount)}</td><td>${fxEscape(request.time || "—")}</td><td class="action-column"><button class="text-action" data-action="${request.source === "product-review" ? "fx-open-combined-product" : "fx-view-bind-request"}" data-id="${request.source === "product-review" ? request.productId : request.id}">${state.portal === "ops" && request.source === "product-review" ? "审核" : "详情"}</button></td></tr>`).join("")
    ...
    if (state.modal === "fx-activation") { const product = products.find(item => item.id === state.drawerProductId); const combined = product?.applicationType === "新建产品并绑定"; if (combined) { const order = orders.find(item => item.no === product.requestedOrderNo); const body = `<div class="confirm-list"><div class="confirm-row"><span>申请类型</span><strong>新建产品并绑定</strong></div><div class="confirm-row"><span>关联订单</span><strong class="mono">${fxEscape(order?.no || product.requestedOrderNo || "—")}</strong></div><div class="confirm-row"><span>客户选定码段</span><strong class="mono">${fxEscape(product.requestedRange || "—")}</strong></div><div class="confirm-row"><span>本次激活数量</span><strong>${formatNumber(product.requestedAmount || 0)} 枚</strong></div></div><p class="modal-note">审核通过后将按客户申请的码段直接激活，运营端不能调整订单、码段或数量。</p>`; return fxModalShell("审核通过并激活", "确认产品资料和绑定设置均符合要求。", body, `<button class="button" data-action="close-modal">取消</button><button class="button primary" data-action="fx-confirm-activation">通过并激活</button>`); } const available = orders.filter(item => item.customer === product.company && fxOrderAvailableAmount(item) > 0).sort((left, right) => Number(right.no === product.preferredOrderNo) - Number(left.no === product.preferredOrderNo)); const preferred = available.find(item => item.no === product.preferredOrderNo) || available[0]; const preferredAvailable = preferred ? fxOrderAvailableAmount(preferred) : 0; return fxModalShell("审核通过并激活", product.preferredOrderNo ? `客户已指定订单 ${product.preferredOrderNo}，仍可在审核时调整。` : "选择订单和本次激活数量，一步完成审核与码段关联。", `<div class="form-grid"><div class="field full bind-product-picker activation-order-picker"><label class="required" for="fx-activation-order-search">激活订单</label><input id="fx-activation-order-search" class="bind-product-search activation-order-search" type="text" value="${fxEscape(preferred?.no || "")}" placeholder="选择激活订单" readonly role="combobox" aria-controls="fx-activation-order-options" aria-expanded="false"><div id="fx-activation-order-options" class="bind-product-options activation-order-options" role="listbox">${available.map(item => { const availableAmount = fxOrderAvailableAmount(item); return `<button type="button" class="bind-product-option activation-order-option" role="option" data-action="fx-select-activation-order" data-no="${fxEscape(item.no)}" data-available="${availableAmount}"><span class="mono">${fxEscape(item.no)}</span><small>可用 ${formatNumber(availableAmount)} 枚</small></button>`; }).join("")}</div><input id="fx-activation-order" type="hidden" value="${fxEscape(preferred?.no || "")}"></div><div class="field full"><label class="required" for="fx-activation-amount">本次激活数量</label><input id="fx-activation-amount" type="number" min="1" max="${preferredAvailable}" value="${preferredAvailable}"><span id="fx-activation-available" class="field-help">可用码量：${formatNumber(preferredAvailable)} 枚</span></div></div>`, `<button class="button" data-action="close-modal">取消</button><button class="button primary" data-action="fx-confirm-activation">通过并激活</button>`); }
    ...
    if (state.modal === "fx-withdraw-decision") { const item = withdrawals[state.selectedWithdrawalIndex]; const reject = state.modalData?.decision === "reject"; return fxModalShell(reject ? "驳回撤回申请" : "通过撤回申请", reject ? "填写驳回原因并通知客户。" : "产品全部已激活码将重置为空白状态。", `<div class="confirm-list"><div class="confirm-row"><span>产品</span><strong>${fxEscape(item.product)}</strong></div><div class="confirm-row"><span>客户</span><strong>${fxEscape(item.customer)}</strong></div><div class="confirm-row"><span>申请原因</span><strong>${fxEscape(item.reason)}</strong></div></div>${reject ? `<div class="field modal-field"><label class="required">驳回原因</label><textarea id="fx-withdraw-reject-reason"></textarea></div>` : ""}`, `<button class="button" data-action="close-modal">取消</button><button class="button ${reject ? "danger" : "primary"}" data-action="fx-confirm-withdraw-decision">确认${reject ? "驳回" : "通过并重置"}</button>`); }
